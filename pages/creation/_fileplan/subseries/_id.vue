<template>
  <div class="about">
    ...
    <v-container>
      ...
      <v-data-table class="elevation-1" :headers="headers" :rows-per-page-items='[50,100,500,{"text":"$vuetify.dataIterator.rowsPerPageAll","value":-1}]' :items="docs" :loading="loading" :search="search">
        <template slot="items" slot-scope="props">
          ...
          <td :class="`${isBold(props.item)} justify-center layout px-0`">
            <v-btn v-if="props.item.disposal != 'None'" depressed small round color="warning" router :to="'templates/' + props.item.number.replace(/\//g, '-')">open<v-icon small>chevron_right</v-icon></v-btn>
          </td>
        </template>
