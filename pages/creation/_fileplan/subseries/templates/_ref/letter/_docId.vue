          ...
          <v-layout row wrap>
            ...
            <v-flex xs8>
              <v-card-text class="black--text ma-0 font-weight-bold text-sm-left">
                ...
                <h4>
                  {{author.Designation}} |
                  <span class="grey--text">{{author.Department}}</span>
                </h4>
              </v-card-text>
            </v-flex>
            <v-flex xs4>
...
import { draftMixin } from "~/mixins/draft";
...
export default {
  ...
  methods: {
    ...signatureHelpers(),
    setRecipients(users) {
      this.doc.body.recipients = users;
    },
    setSigners(users) {
  ...
  created() {
    this.doc.template = "letter"
    this.doc.body.address = "<h3>The Head of Department</h3><h3>Limpopo Provincial Treasury</h3><h3>Polokwane</h3><h3>0700</h3>"
    console.log(this.doc.body.address);
    console.log('letter');
  }
};
</script>
